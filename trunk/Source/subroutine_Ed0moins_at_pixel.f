...
C Lecture de la LUT
C
      SUBROUTINE read_ed0moins_LUT(lutpath_length, LUTPATH,LUT)
      
      parameter (nwl=83, nthetas=19,   nozone=8, ntaucl=8)

      integer lutpath_length
      real*4 LUT(nwl, nthetas, nozone, ntaucl)
      character LUTPATH*500
      integer i,j,k,l
      character(len=lutpath_length) :: lutpath_clean
      logical :: file_exists

      lutpath_clean = LUTPATH(1:lutpath_length)

      INQUIRE(FILE=LUTPATH, EXIST=file_exists)
      IF (.NOT.file_exists) THEN
         print *, "File not found: ", lutpath_clean
      END IF
      
      OPEN(10, file=lutpath_clean, status="old",
